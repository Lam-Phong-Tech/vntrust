...
export default function QRPrintPage() {
  ...
  const [confirmModal, setConfirmModal] = useState<{ type: 'regenerate' | 'delete', uid: string } | null>(null);
  ...
  const executeAction = async () => {
    ...
    try {
      if (type === 'delete') {
        const res = await fetch(`/api/inventory/${id}/qr?uid=${uid}`, { method: 'DELETE' });
        if (!res.ok) throw new Error((await res.json()).error);
        showToast("✓ Đã xóa mã QR", true);
            ...
            <div className="p-6 text-center">
              ...
              <p className="text-sm text-slate-500 mt-2">
                {confirmModal.type === 'delete' 
                  ? 'Mã QR này sẽ bị xóa vĩnh viễn khỏi hệ thống.' 
