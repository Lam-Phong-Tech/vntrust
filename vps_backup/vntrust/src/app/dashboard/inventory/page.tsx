        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setModal(null)}>
          <div className="bg-[#1a2235] border border-white/10 rounded-3xl shadow-2xl w-full max-w-md p-8" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setModal(null)} className="text-slate-400 hover:text-white"><span className="material-symbols-outlined">close</span></button>
            </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="date" value={form.hanDung || ""}
                    min={(() => {
                      const todayStr = new Date().toISOString().split('T')[0];
                      if (!form.ngaySanXuat) return todayStr;
                      const d = new Date(form.ngaySanXuat); d.setDate(d.getDate() + 1);
                      const nsxNext = d.toISOString().split('T')[0];
                      return nsxNext > todayStr ? nsxNext : todayStr;
                    })()}
