  ...
  return (
    <div className="min-h-[calc(100vh-80px)] pb-12 flex flex-col items-center pt-8 px-4 relative">
      <div className="fixed inset-0 pointer-events-none overflow-hidden flex items-center justify-center">
        <div className="w-[60%] h-[60%] rounded-full bg-cyan-500/10 blur-[150px]"></div>
      </div>

      <div className="w-full max-w-4xl relative z-10 flex flex-col items-center">
        {/* Navigation / Back */}
        <div className="w-full mb-6">
          <Link href="/verify" className="inline-flex items-center gap-2 text-sm font-bold text-on-surface-variant hover:text-cyan-400 transition">
        ...
        <div className="glass-panel border border-outline-variant/15 rounded-3xl p-8 w-full shadow-2xl bg-surface-container-lowest/80 backdrop-blur-xl">
          <div className="flex flex-col md:flex-row items-center gap-6 mb-10 pb-8 border-b border-outline-variant/20">
            <div className="w-16 h-16 rounded-2xl bg-cyan-500/10 flex items-center justify-center text-cyan-400 shrink-0">
              <span className="material-symbols-outlined text-4xl">document_scanner</span>
            </div>
            <div>
              <h2 className="text-3xl font-black font-headline text-on-surface mb-2">{t("aidoc_title")}</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div className="flex flex-col h-full">
              <div 
                className={`flex-1 border-2 border-dashed rounded-3xl flex flex-col items-center justify-center p-8 transition-all min-h-[350px] relative overflow-hidden group ${
                  preview ? 'border-cyan-500/30 bg-cyan-500/5' : 'border-outline-variant/30 hover:border-cyan-500/40 hover:bg-surface-container'
                }`}
              ...
              >
                {preview ? (
                    ...
                    {status === "analyzing" && (
                      <>
                        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-10 flex flex-col items-center justify-center">
                          <span className="material-symbols-outlined text-cyan-400 animate-spin text-5xl mb-4">progress_activity</span>
                          <p className="text-cyan-400 font-bold tracking-widest uppercase">{t("aidoc_analyzing")}</p>
                        </div>
                        {/* Scanning Laser */}
                        <div className="absolute top-0 left-0 w-full h-1 bg-cyan-400 shadow-[0_0_15px_#22d3ee] z-20 animate-[scan_2s_ease-in-out_infinite]" style={{ animationDirection: 'alternate' }}></div>
                      </>
                    )}
                  ...
                  <>
                    ...
                    <h3 className="font-bold text-on-surface mb-2 font-headline">{t("aidoc_drag")}</h3>
              ...
              {preview && status === "idle" && (
                <button onClick={triggerAnalysis} className="mt-4 w-full bg-cyan-500 hover:bg-cyan-400 text-white shadow-lg shadow-cyan-500/20 rounded-2xl py-4 font-bold tracking-wide uppercase transition-all flex items-center justify-center gap-2">
            ...
            <div className="flex flex-col bg-surface-container-low rounded-3xl p-6 border border-outline-variant/10">
              <h3 className="font-black font-headline text-lg mb-6 flex items-center gap-2 text-on-surface">
              ...
              {status === "idle" && preview && (
                <div className="flex-1 flex items-center justify-center">
                  <p className="text-sm text-cyan-400 animate-pulse">{t("aidoc_ready")}</p>
                </div>
              )}
              ...
              {status === "success" && extracted && (
                <div className="flex-1 space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="bg-emerald-500/10 border border-emerald-500/30 p-4 rounded-2xl flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-emerald-400 font-bold uppercase tracking-wide text-sm font-headline">{t("aidoc_valid")}</h4>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="bg-surface-container rounded-xl p-3 flex flex-col gap-2 text-sm">
                      ...
                      <div className="bg-black/40 rounded-lg p-3 font-mono text-xs text-cyan-400 whitespace-pre-line leading-relaxed border border-cyan-500/20">
                        {extracted.rawText}
                      </div>
                    </div>
                  </div>
              ...
              {status === "fake" && (
                <div className="flex-1 space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="bg-red-500/10 border border-red-500/30 p-4 rounded-2xl flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-red-400 font-bold uppercase tracking-wide text-sm font-headline">{t("aidoc_fake")}</h4>
