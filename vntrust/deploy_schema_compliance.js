...
conn.on('ready', () => {
  ...
  conn.sftp((err, sftp) => {
    if (err) throw err;
    const localPath = path.join(__dirname, 'prisma/schema.prisma');
    const remotePath = `/var/www/vntrust/prisma/schema.prisma`;
    ...
    sftp.fastPut(localPath, remotePath, (err) => {
      ...
      console.log('Schema uploaded. Running prisma generate, push & build...');
      conn.exec('cd /var/www/vntrust && npx prisma generate && npm run build && pm2 restart vntrust', (err, stream) => {
