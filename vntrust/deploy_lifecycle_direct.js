...
conn.on('ready', () => {
  ...
  conn.sftp((err, sftp) => {
    if (err) throw err;
    const localPath = path.join(__dirname, 'src/app/api/lifecycle-check/route.ts');
    const remotePath = `/var/www/vntrust/src/app/api/lifecycle-check/route.ts`;
    ...
    sftp.fastPut(localPath, remotePath, (err) => {
      ...
      conn.exec('cd /var/www/vntrust && npm run build && pm2 restart vntrust', (err, stream) => {
        ...
        stream.on('close', () => {
          ...
          conn.exec('curl -s -X GET "http://127.0.0.1:3001/api/lifecycle-check?secret=vntrust-cron-key"', (err2, stream2) => {
