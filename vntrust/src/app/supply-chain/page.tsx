      ...
      <main className="mx-auto max-w-7xl w-full flex-1 p-4 sm:p-8 lg:p-12 overflow-x-hidden min-h-[calc(100vh-80px)] transparent">
        <header className="flex justify-between items-center mb-10 pb-4 border-b border-outline-variant/10">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold font-display tracking-tighter text-white">{t("sc_title")}</h1>
          </div>
        ...
        <div className="space-y-8">
          ...
          <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="bg-white/5 glass-panel text-white p-5 sm:p-6 rounded-2xl shadow-sm border-l-4 border-primary relative overflow-hidden group">
              <div className="absolute -right-4 -top-4 w-24 h-24 bg-primary/5 rounded-full scale-150 transition-transform group-hover:scale-175 pointer-events-none"></div>
              <p className="text-xs sm:text-sm font-bold text-secondary uppercase tracking-tighter mb-2">{t("sc_total_auth")}</p>
              <div className="flex flex-wrap items-end gap-x-3 gap-y-1.5 min-w-0">
                <h3 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold font-display text-white leading-none break-words">{totalAuths.toLocaleString("en-US")}</h3>
                {(() => {
                  ...
                  return (
                    <span className={`text-[10px] sm:text-xs font-bold flex items-center px-2 py-0.5 rounded-full whitespace-nowrap transition-all ${pos ? "text-emerald-700 bg-emerald-50" : "text-red-600 bg-red-50"}`}>
                      <span className="material-symbols-outlined text-xs">{pos ? "trending_up" : "trending_down"}</span> {pos ? "+" : ""}{rate}%
                    </span>
                  );
                })()}
              </div>
              <p className="text-[10px] sm:text-[11px] text-outline mt-3 sm:mt-4 font-mono transition-opacity truncate">HASH: {generateId()}...{generateId().substring(0, 4)}</p>
            </div>

            <div className="bg-white/5 glass-panel text-white p-5 sm:p-6 rounded-2xl shadow-sm border-l-4 border-error relative overflow-hidden group">
              <div className="absolute -right-4 -top-4 w-24 h-24 bg-error/5 rounded-full scale-150 transition-transform group-hover:scale-175 pointer-events-none"></div>
              <p className="text-xs sm:text-sm font-bold text-secondary uppercase tracking-tighter mb-2">{t("sc_fake_prev")}</p>
              <div className="flex flex-wrap items-end gap-x-3 gap-y-1.5 min-w-0">
                <h3 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold font-display text-white leading-none break-words">{preventions.toLocaleString("en-US")}</h3>
                <span className="text-error text-[10px] sm:text-xs font-bold flex items-center bg-error-container px-2 py-0.5 rounded-full whitespace-nowrap">
                  <span className="material-symbols-outlined text-xs">security</span> {t("sc_active_badge")}
                </span>
              </div>
              <p className="text-[10px] sm:text-[11px] text-outline mt-3 sm:mt-4 truncate">{t("sc_ai_detect")}</p>
            </div>

            <div className="bg-white/5 glass-panel text-white p-5 sm:p-6 rounded-2xl shadow-sm border-l-4 border-tertiary-container relative overflow-hidden group">
              <div className="absolute -right-4 -top-4 w-24 h-24 bg-tertiary-container/5 rounded-full scale-150 transition-transform group-hover:scale-175 pointer-events-none"></div>
              <p className="text-xs sm:text-sm font-bold text-secondary uppercase tracking-tighter mb-2">{t("sc_efficiency")}</p>
              <div className="flex flex-wrap items-end gap-x-3 gap-y-1.5 min-w-0">
                <h3 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold font-display text-white leading-none">{safeScoreDisplay}<span className="text-lg sm:text-xl lg:text-2xl">%</span></h3>
                <span className="text-emerald-700 text-[10px] sm:text-xs font-bold flex items-center bg-emerald-50 px-2 py-0.5 rounded-full whitespace-nowrap">
                  {t("sc_optimal")}
                </span>
              </div>
              <p className="text-[10px] sm:text-[11px] text-outline mt-3 sm:mt-4 truncate">{t("sc_latency")} {latency}ms (Live)</p>
            </div>
          </section>
            ...
            <div className="col-span-12 lg:col-span-8 bg-white/5 glass-panel text-white p-8 rounded-2xl shadow-sm border border-outline-variant/10 transition-all overflow-hidden">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                  <h4 className="text-xl font-bold font-display text-white flex items-center gap-2">
                    {t("sc_auth_speed")} <span className="w-2 h-2 rounded-full bg-[#4A7C5C] animate-pulse"></span>
                  </h4>
              ...
              <div className="relative w-full h-[200px]">
                <svg viewBox="0 0 800 200" className="w-full h-full" preserveAspectRatio="none">
                  ...
                  {chartPoints.length === 0 && (
                    <text x="400" y="100" textAnchor="middle" fill="#64748b" fontSize="14">{t("sc_no_data") || "Chưa có dữ liệu quét"}</text>
                  )}
                </svg>
              ...
              <div className="flex items-center justify-center mt-6 gap-8">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-[#4A7C5C] inline-block"></span><span className="text-slate-300">{t("sc_total_auth")}: <strong className="text-white">{totalAuths.toLocaleString()}</strong></span></div>
                  ...
                  <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-[#C8A557] inline-block"></span><span className="text-slate-300">{t("sc_latency")}: <strong className="text-white">{latency}ms</strong></span></div>
                </div>
              </div>
            </div>
            ...
            <div className="col-span-12 lg:col-span-4 flex flex-col gap-6">
              ...
              <div className="bg-white/5 glass-panel text-white p-6 rounded-2xl shadow-sm border border-outline-variant/10 flex-1">
                <div className="flex justify-between items-center mb-4">
                  <div>
                    <h4 className="text-lg font-bold font-display text-white">{t("sc_scan_activity")}</h4>
                ...
                <div className="relative w-full h-[200px] bg-white/10 rounded-xl overflow-hidden group">
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-24 h-24 bg-[#4A7C5C]/30 rounded-full blur-2xl animate-ping opacity-50"></div>
              ...
              <div className="bg-white/5 glass-panel text-white p-5 rounded-2xl border border-outline-variant/10">
                <p className="text-xs text-secondary uppercase tracking-widest mb-1">{(t("sc_today") || "Hôm nay").replace(":", "")}</p>
                <p className="text-3xl font-extrabold text-white">{todayAuths.toLocaleString()}</p>
                <p className="text-xs text-slate-400 mt-1">{t("sc_scans_count") || "lượt xác thực"}</p>
              </div>
            </div>
          </section>
          ...
          <section className="bg-white/5 glass-panel text-white p-8 rounded-2xl shadow-sm border border-outline-variant/10 flex flex-col relative overflow-hidden">
            ...
            <h4 className="text-xl font-bold font-display text-white mb-1 flex items-center justify-between">
              {t("sc_latest_ledger")}
                  ...
                  <div key={log.id} className="flex gap-3 p-4 bg-white/5 rounded-xl border border-white/10 hover:bg-white/10 transition">
                    ...
                    <div className="min-w-0">
                      ...
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded mt-1 inline-block ${
                        log.type === "error" ? "bg-red-500/20 text-red-400" : "bg-white/10 text-[#C8A557]"
                      }`}>{logTime}</span>
                    </div>
                  </div>
                );
        ...
        {/* Floating Action — mobile cần bottom > 64px (mobile nav height) */}
        <div className="fixed right-4 lg:right-8 z-40 bottom-[80px] lg:bottom-8">
          <div className="flex items-center gap-2 lg:gap-3 bg-slate-900 text-white pl-3 lg:pl-4 pr-4 lg:pr-6 py-2 lg:py-3 rounded-full shadow-2xl border border-slate-700">
            <div className="w-6 h-6 lg:w-8 lg:h-8 rounded-full bg-[#4A7C5C] flex items-center justify-center shrink-0">
              <span className="material-symbols-outlined text-white text-xs lg:text-sm">bolt</span>
            </div>
            <div className="text-left">
              <p className="text-[9px] lg:text-[10px] font-bold uppercase tracking-tighter text-[#6FB585]">{t("sc_ai_running")}</p>
              <p className="text-[10px] lg:text-xs font-medium text-slate-300">{t("sc_scanning")} {(1.5 + Math.random()).toFixed(1)}k {t("sc_data_per_sec")}</p>
            </div>
          </div>
        </div>
