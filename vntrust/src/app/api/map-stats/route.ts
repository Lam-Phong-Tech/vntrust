...
export async function GET(_req: NextRequest) {
  try {
    ...
    const markers = MARKER_BASE.map(m => {
      // Tìm trong scan counts theo nhiều alias — dedupe per city để tránh double-count
      // (alias overlap: "TP.HCM" và "HCM" cùng match → count 2 lần)
      let realScans = 0;
      let realFake = 0;
      ...
      for (const [city, count] of Object.entries(scanCounts)) {
        if (matchedCities.has(city)) continue;
        const cityLower = city.toLowerCase();
        if (aliases.some(a => cityLower.includes(a.toLowerCase()) || a.toLowerCase().includes(cityLower))) {
          realScans += count;
          matchedCities.add(city);
        }
      }
      const matchedFakeCities = new Set<string>();
      for (const [city, count] of Object.entries(fakeCounts)) {
        if (matchedFakeCities.has(city)) continue;
        const cityLower = city.toLowerCase();
        if (aliases.some(a => cityLower.includes(a.toLowerCase()) || a.toLowerCase().includes(cityLower))) {
          realFake += count;
          matchedFakeCities.add(city);
        }
      }
