...
export async function GET(req: NextRequest) {
  try {
    ...
    // ── PHÂN HỆ 3 — Admin: Quản lý & Phân tích dữ liệu báo cáo ────────────────
    if (type === 'admin_reports') {
      if (userRole !== 'admin') {
        return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
      }

      // 3.1 — Dashboard tổng quan
      const totalReports = await prisma.canhBao.count({
        where: { loai: 'NGUOI_DUNG_BAO_CAO', thoiGian: { gte: periodStart } },
      });
      const openReports = await prisma.canhBao.count({
        where: { loai: 'NGUOI_DUNG_BAO_CAO', trangThai: 'open', thoiGian: { gte: periodStart } },
      });
      const reviewingReports = await prisma.canhBao.count({
        where: { loai: 'NGUOI_DUNG_BAO_CAO', trangThai: 'reviewing', thoiGian: { gte: periodStart } },
      });
      const closedReports = await prisma.canhBao.count({
        where: { loai: 'NGUOI_DUNG_BAO_CAO', trangThai: 'closed', thoiGian: { gte: periodStart } },
      });
      // Tỷ lệ phát hiện hàng giả = đã đóng / tổng (proxy cho confirmed)
      const fakeConfirmRate = totalReports > 0 ? ((closedReports / totalReports) * 100).toFixed(1) : '0.0';

      // Top sản phẩm bị báo cáo
      const topReported = await prisma.canhBao.groupBy({
        by: ['uid'],
        where: { loai: 'NGUOI_DUNG_BAO_CAO', uid: { not: null }, thoiGian: { gte: periodStart } },
        _count: true,
        orderBy: { _count: { uid: 'desc' } },
        take: 10,
      });

      // 3.2 — Theo loại phản ánh (4 nhóm PHÂN HỆ 2)
      const byLoaiPhanAnh = await prisma.canhBao.groupBy({
        by: ['loaiPhanAnh'],
        where: { loai: 'NGUOI_DUNG_BAO_CAO', thoiGian: { gte: periodStart } },
        _count: true,
      });

      // Theo nơi mua (kênh bán lẻ)
      const byNoiMua = await prisma.canhBao.groupBy({
        by: ['noiMua'],
        where: { loai: 'NGUOI_DUNG_BAO_CAO', noiMua: { not: null }, thoiGian: { gte: periodStart } },
        _count: true,
      });

      // Theo mức độ
      const byMucDo = await prisma.canhBao.groupBy({
        by: ['mucDo'],
        where: { loai: 'NGUOI_DUNG_BAO_CAO', thoiGian: { gte: periodStart } },
        _count: true,
      });

      // Theo thời gian (xu hướng 7 hoặc 30 ngày gần nhất)
      const reportTrend = [];
      ...
      for (let i = days - 1; i >= 0; i--) {
        ...
        const count = await prisma.canhBao.count({
          where: { loai: 'NGUOI_DUNG_BAO_CAO', thoiGian: { gte: d, lte: dEnd } },
        });
        reportTrend.push({
      ...
      return NextResponse.json({
        period,
        // 3.1 — Tổng quan
        totalReports, openReports, reviewingReports, closedReports,
        fakeConfirmRate,
        topReported: topReported.map(r => ({ uid: r.uid, count: r._count })),
        // 3.2 — Phân tích đa chiều
        byLoaiPhanAnh: byLoaiPhanAnh.map(r => ({ key: r.loaiPhanAnh, count: r._count })),
        byNoiMua: byNoiMua.map(r => ({ key: r.noiMua, count: r._count })),
        byMucDo: byMucDo.map(r => ({ key: r.mucDo, count: r._count })),
        // Trend
        reportTrend,
      });
    }
