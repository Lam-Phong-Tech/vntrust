...
export async function GET(req: NextRequest) {
  try {
    // P0.3 — Đọc role + DN từ cookie (không tin query param để tránh spoof)
    ...
    const where: any = loHangId
      ...
      ? {}
      : userDN
        ? { loHang: { sanPham: { doanhNghiepId: userDN } } }
        : { id: 'none' };
...
export async function POST(req: NextRequest) {
  try {
    ...
    // UC03 sub-role: FAIL-CLOSED — missing cookie → lookup DB
    ...
    if (userRole !== 'admin' && !['company_admin', 'warehouse'].includes(vaiTroCty || 'viewer')) {
      return NextResponse.json({ error: `Forbidden: vai trò nội bộ "${vaiTroCty}" không có quyền ghi giao dịch kho (cần Quản trị DN hoặc Nhân viên kho)` }, { status: 403 });
    }
