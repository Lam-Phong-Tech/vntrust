...
const PROB_STYLE: Record<string, string> = {
  low:    "text-emerald-300 bg-[#4A7C5C]/15 border-[#4A7C5C]/30",
...
const IMPACT_STYLE: Record<string, string> = {
  ...
  medium:   "text-amber-300 bg-[#C8A557]/15 border-[#C8A557]/30",
  high:     "text-red-300 bg-red-500/15 border-red-500/30",
  critical: "text-violet-300 bg-[#C8A557]/15 border-[#C8A557]/30",
};
const RISK_STATUS: Record<string, { cls: string; label: string }> = {
  open:      { cls: "text-red-300 bg-red-500/15 border-red-500/30",       label: "Mở" },
  mitigated: { cls: "text-emerald-300 bg-[#4A7C5C]/15 border-[#4A7C5C]/30", label: "Đã giảm thiểu" },
...
const CONSTRAINT_STATUS: Record<string, { cls: string; label: string; icon: string }> = {
  compliant:   { cls: "text-emerald-300 bg-[#4A7C5C]/15 border-[#4A7C5C]/30", label: "Tuân thủ",     icon: "check_circle" },
    ...
    <div className="min-h-[calc(100vh-80px)] p-6 md:p-10 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-4 gap-3 mb-8">
        {[
          ...
          { label: "Đã giảm thiểu",      value: mitigatedRisks, color: "text-[#6FB585]", icon: "shield_check" },
          { label: "Risk Score TB",      value: avgScore,       color: avgScore >= 6 ? "text-red-400" : avgScore >= 4 ? "text-[#C8A557]" : "text-[#6FB585]", icon: "speed" },
          { label: "Ràng buộc",          value: CONSTRAINTS.length, color: "text-[#C8A557]", icon: "rule" },
        ].map((s, i) => (
        ...
        <div className="space-y-4">
          ...
          <div className="glass-panel border border-red-500/20 rounded-2xl p-5">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { label: "Critical (8–9)",  risks: RISKS.filter(r => r.score >= 8), cls: "border-[#C8A557]/30 bg-[#C8A557]/10" },
              ...
              return (
                <div key={risk.id} className="glass-panel border border-white/10 rounded-2xl p-5">
                  ...
                  <div className="flex items-start gap-2 p-3 bg-white/5 rounded-xl text-xs text-slate-300">
                    <span className="material-symbols-outlined text-[14px] text-[#6FB585] shrink-0 mt-0.5">health_and_safety</span>
      ...
      {activeTab === "assumption" && (
        <div className="space-y-3">
          <div className="p-4 bg-[#C8A557]/10 border border-[#C8A557]/20 rounded-xl text-xs text-blue-300 flex gap-2 mb-4">
          ...
          {ASSUMPTIONS.map((a, i) => (
            <div key={a.id} className="glass-panel border border-white/10 rounded-2xl p-4 flex items-start gap-4">
              <span className="w-8 h-8 rounded-lg bg-[#C8A557]/20 border border-[#C8A557]/30 flex items-center justify-center text-[#C8A557] font-black text-sm shrink-0">
                {i + 1}
              </span>
              <div>
