...
const ACTOR_COLORS: Record<string, { bg: string; text: string; label: string }> = {
  NSX: { bg: "bg-[#C8A557]/20 border-[#C8A557]/30",   text: "text-blue-300",   label: "Nhà sản xuất" },
  NPP: { bg: "bg-[#C8A557]/20 border-[#C8A557]/30",    text: "text-cyan-300",   label: "Nhà phân phối" },
  NTD: { bg: "bg-[#4A7C5C]/20 border-[#4A7C5C]/30", text: "text-emerald-300", label: "Người tiêu dùng" },
  ADM: { bg: "bg-[#C8A557]/20 border-[#C8A557]/30", text: "text-violet-300", label: "Admin" },
};

const UC_STATUS: Record<string, { icon: string; cls: string; label: string }> = {
  done:    { icon: "check_circle", cls: "text-[#6FB585]", label: "Hoàn thành" },
      ...
      {activeTab === "gate" && (
        <div className="space-y-6">
          {GATES.map((gate, gi) => {
            ...
            const colorMap: Record<string,string> = { cyan:"border-[#C8A557]/30 bg-[#C8A557]/5", purple:"border-[#C8A557]/30 bg-[#C8A557]/5", emerald:"border-[#4A7C5C]/30 bg-[#4A7C5C]/5" };
            const textMap: Record<string,string> = { cyan:"text-[#C8A557]", purple:"text-[#C8A557]", emerald:"text-[#6FB585]" };
            return (
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${gate.status === "in_progress" ? "bg-[#C8A557] text-white" : gate.status === "done" ? "bg-[#4A7C5C] text-white" : "bg-white/10 text-slate-400"}`}>
                  ...
                  <div className="text-right">
                    <p className={`text-3xl font-black ${pct === 100 ? "text-[#6FB585]" : pct >= 50 ? "text-[#C8A557]" : "text-red-400"}`}>{pct}%</p>
                    <p className="text-xs text-slate-400">{passCount}/{gate.criteria.length} điều kiện</p>
                  </div>
                </div>

                <div className="w-full bg-white/10 rounded-full h-2 mb-4">
                  <div className={`h-2 rounded-full transition-all ${pct===100?"bg-[#4A7C5C]":pct>=50?"bg-[#C8A557]":"bg-red-500"}`} style={{ width: `${pct}%` }} />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-4">
                  {gate.criteria.map((c, i) => (
                    <div key={i} className={`flex items-center gap-2 p-2.5 rounded-xl text-sm ${c.pass ? "bg-[#4A7C5C]/10 border border-[#4A7C5C]/20" : "bg-red-500/10 border border-red-500/20"}`}>
                      <span className={`material-symbols-outlined text-[16px] shrink-0 ${c.pass ? "text-[#6FB585]" : "text-red-400"}`}>
                    ...
                    <tr key={ac.id} className="hover:bg-white/5 transition">
                      ...
                      <td className="px-5 py-3 text-center">
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold border ${
                          ac.pass === true ? "text-emerald-300 bg-[#4A7C5C]/15 border-[#4A7C5C]/30" :
      ...
      {activeTab === "uc" && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: "Hoàn thành", count: doneCount,    cls: "text-[#6FB585] bg-[#4A7C5C]/10 border-[#4A7C5C]/20" },
