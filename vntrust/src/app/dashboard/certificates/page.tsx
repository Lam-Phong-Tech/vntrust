...
const CERT_COLORS: Record<string, string> = {
  ISO:     "text-blue-300 bg-[#C8A557]/15 border-[#C8A557]/30",
  HACCP:   "text-green-300 bg-[#4A7C5C]/15 border-[#4A7C5C]/30",
  GMP:     "text-cyan-300 bg-[#C8A557]/15 border-[#C8A557]/30",
  CFS:     "text-purple-300 bg-[#C8A557]/15 border-[#C8A557]/30",
  FDA:     "text-rose-300 bg-rose-500/15 border-rose-500/30",
  HALAL:   "text-emerald-300 bg-[#4A7C5C]/15 border-[#4A7C5C]/30",
    ...
    <div className="min-h-[calc(100vh-80px)] p-6 md:p-10 max-w-7xl mx-auto">
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#4A7C5C]/20 border border-[#4A7C5C]/30 flex items-center justify-center">
              <span className="material-symbols-outlined text-[#6FB585]">workspace_premium</span>
            </div>
            <div>
              <h1 className="text-2xl font-black text-white font-display">Chứng nhận Sản phẩm</h1>
              <p className="text-sm text-slate-400">BR-02: Quản lý ISO, HACCP, GMP, CFS, FDA... theo sản phẩm</p>
            </div>
          </div>
        </div>
        <button onClick={() => setModal(true)}
          className="flex items-center gap-2 px-4 py-2.5 bg-[#C8A557]/20 hover:bg-[#C8A557]/30 text-cyan-300 border border-[#C8A557]/30 rounded-xl font-bold text-sm transition active:scale-95">
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        {[
          ...
          { label: "Sắp hết hạn (90 ngày)", value: expiringCount, icon: "schedule", color: "text-[#C8A557]" },
      ...
      {loading ? (
        <div className="flex items-center justify-center h-48">
          <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-[#4A7C5C]" />
        </div>
      ) : certs.length === 0 ? (
        <div className="text-center py-16 text-slate-400">
          <span className="material-symbols-outlined text-5xl mb-3 block">workspace_premium</span>
          <p className="font-bold text-white mb-2">{tr("Chưa có chứng nhận nào", "No certificates yet")}</p>
          <p className="text-sm mb-5">Thêm chứng nhận ISO, HACCP, GMP... để tăng uy tín sản phẩm</p>
          <button onClick={() => setModal(true)} className="px-6 py-2.5 bg-[#C8A557]/20 text-cyan-300 border border-[#C8A557]/30 rounded-xl font-bold hover:bg-[#C8A557]/30 transition">
            + Thêm chứng nhận
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {certs.map(cert => {
            ...
            return (
              <div key={cert.id} className={`glass-panel border rounded-2xl p-5 flex flex-col gap-3 transition hover:border-white/20 ${isExpired ? 'border-red-500/30 bg-red-900/10' : expiringSoon ? 'border-[#C8A557]/20' : 'border-white/10'}`}>
                <div className="flex items-start justify-between">
                  ...
                  {isExpired && (
                    <span className="text-xs px-2 py-0.5 bg-red-500/20 text-red-400 border border-red-500/30 rounded-full font-bold">{tr("Hết hạn", "Expired")}</span>
                  )}
                  {!isExpired && expiringSoon && (
                    <span className="text-xs px-2 py-0.5 bg-[#C8A557]/20 text-[#C8A557] border border-[#C8A557]/30 rounded-full font-bold">
                      Còn {daysLeft} ngày
                    </span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs text-slate-400">
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-slate-500 mb-0.5">{tr("Ngày cấp", "Issue date")}</p>
                    <p className="text-white font-medium">{new Date(cert.ngayCap).toLocaleDateString("vi-VN")}</p>
                  </div>
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-slate-500 mb-0.5">{tr("Hết hạn", "Expires")}</p>
                    <p className={`font-medium ${isExpired ? "text-red-400" : expiringSoon ? "text-[#C8A557]" : "text-white"}`}>
      ...
      {/* Add modal */}
      {modal && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center md:p-4 pb-[80px] md:pb-4 bg-black/60 backdrop-blur-sm" onClick={() => setModal(false)}>
          <div className="bg-[#0B1623] border border-white/10 rounded-t-3xl md:rounded-3xl w-full max-w-lg shadow-2xl max-h-[calc(100dvh-160px)] md:max-h-[88dvh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="overflow-y-auto p-6 sm:p-7 flex-1 min-h-0">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-10 h-10 rounded-xl bg-[#4A7C5C]/20 flex items-center justify-center">
                <span className="material-symbols-outlined text-[#6FB585]">workspace_premium</span>
              </div>
              <h3 className="text-lg font-bold text-white">Thêm Chứng nhận</h3>
            </div>

            <div className="p-3 bg-[#C8A557]/10 border border-[#C8A557]/20 rounded-xl mb-5 text-xs text-blue-300 flex gap-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={form.loai} onChange={e => setForm(f => ({ ...f, loai: e.target.value }))}
                    className="w-full bg-white/5 border border-white/20 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-[#4A7C5C]">
                ...
                <div>
                  ...
                  <input value={form.soChungNhan || ""} onChange={e => setForm(f => ({ ...f, soChungNhan: e.target.value }))}
                    ...
                    className="w-full bg-white/5 border border-white/20 rounded-xl px-3 py-2.5 text-sm text-white font-mono placeholder:text-slate-500 focus:outline-none focus:border-[#4A7C5C]" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="date" value={form.ngayCap || ""} onChange={e => setForm(f => ({ ...f, ngayCap: e.target.value }))}
                    required className="w-full bg-white/5 border border-white/20 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-[#4A7C5C]" />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1.5">
              ...
              <div>
                ...
                <input value={form.toChucCap || ""} onChange={e => setForm(f => ({ ...f, toChucCap: e.target.value }))}
                  ...
                  className="w-full bg-white/5 border border-white/20 rounded-xl px-3 py-2.5 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-[#4A7C5C]" />
              </div>
                  ...
                  {previewImg ? (
                    <div className="relative">
                      ...
                      <button type="button" onClick={() => { setPreviewImg(""); setForm(f => ({ ...f, hinhAnhUrl: "" })); }}
                        className="text-xs text-red-400 hover:text-red-300 underline">{tr("Xóa ảnh", "Remove image")}</button>
                    </div>
                  ) : (
            ...
            {/* Sticky footer cho mobile — không bị che bởi bottom nav */}
            <div className="flex gap-3 px-6 sm:px-7 py-4 border-t border-white/10 bg-[#0B1623] shrink-0" style={{ paddingBottom: 'max(1rem, env(safe-area-inset-bottom))' }}>
              <button type="button" onClick={() => setModal(false)}
                className="flex-1 py-3 border border-white/20 rounded-xl text-sm font-bold text-slate-300 hover:bg-white/5 transition">{tr("Huỷ", "Cancel")}</button>
              <button type="button" onClick={handleSubmit as any} disabled={submitting}
                ...
                {tr("Lưu chứng nhận", "Save certificate")}
              </button>
            </div>
          </div>
        </div>
