      ...
      <main className="mx-auto max-w-7xl w-full flex-1 p-8 lg:p-12 overflow-x-hidden min-h-[calc(100vh-80px)] transparent">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-4">
          <div>
            ...
            <h1 className="font-display text-4xl font-extrabold text-white tracking-tight">{t("qual_post")}</h1>
                ...
                <tbody className="divide-y divide-slate-50">
                  {paginatedHk.map(hk => (
                    <tr key={hk.id} className="hover:transparent/70 transition-colors">
                      ...
                      <td className="px-4 sm:px-6 py-4">
                        {hk.ketQua === 'dambao' ? (
                          <div className="flex items-center gap-1 text-[#6FB585] text-sm font-bold whitespace-nowrap">
                          ...
                          {hk.trangThaiXacMinh === 'pending' ? (
                            <div className="flex gap-2">
                              <button
                                ...
                                className="px-3 py-1.5 bg-[#4A7C5C]/20 text-emerald-300 border border-[#4A7C5C]/30 rounded-lg text-xs font-bold hover:bg-[#4A7C5C]/30 transition whitespace-nowrap flex items-center gap-1"
                              >
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start mb-2">
                      <span className="inline-block px-3 py-1 bg-[#4A7C5C]/10 text-[#6FB585] font-bold text-[10px] tracking-widest uppercase rounded-full border border-[#4A7C5C]/20">{cn.loai}</span>
                    ...
                    <h3 className="font-display font-bold text-xl text-white mb-1">{cn.soChungNhan}</h3>
                  ...
                  {cn.trangThaiDuyet === 'pending' && typeof window !== 'undefined' && localStorage.getItem('userRole') === 'admin' && (
                    <div className="flex gap-2 mt-4 pt-4 border-t border-white/10">
                      <button onClick={() => handleApprove(cn.id, 'approve')} className="px-4 py-1.5 bg-[#4A7C5C] hover:bg-[#4A7C5C] text-white rounded-lg text-sm font-bold transition">Phê duyệt</button>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setModal(false)}>
          <div className="bg-white/5 glass-panel text-white rounded-3xl shadow-2xl w-full max-w-lg p-8" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold font-display">{t("hk_upload_btn")}</h2>
                ...
                <div>
                  ...
                  <input type="date" value={form.ngayLayMau || ""} onChange={e => setForm(f => ({ ...f, ngayLayMau: e.target.value }))}
                    max={(() => {
                      ...
                      if (!form.ngayPhanTich) return todayStr;
                      return form.ngayPhanTich < todayStr ? form.ngayPhanTich : todayStr;
                    })()}
                    className="w-full border border-white/20 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition" />
                </div>
                <div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setVerifyModal(null)}>
          <div className="bg-[#0B1623] border border-white/10 rounded-3xl p-7 w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-5">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${verifyModal.action === 'verify' ? 'bg-[#4A7C5C]/20' : 'bg-red-500/20'}`}>
                <span className={`material-symbols-outlined ${verifyModal.action === 'verify' ? 'text-[#6FB585]' : 'text-red-400'}`}>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleVerifyHauKiem} disabled={verifying}
                className={`flex-1 py-3 rounded-xl text-sm font-bold transition disabled:opacity-50 flex items-center justify-center gap-2 ${verifyModal.action === 'verify' ? 'bg-[#4A7C5C] hover:bg-[#4A7C5C] text-white' : 'bg-red-500/80 hover:bg-red-500 text-white'}`}>
