...
const LOAI_GD_KEYS: Record<string, { labelKey: string; icon: string; cls: string; arrow: string }> = {
  NHAP_KHO:   { labelKey: "wh_filter_in",   icon: "download",    cls: "text-emerald-300 bg-[#4A7C5C]/15 border-[#4A7C5C]/30", arrow: "↓" },
  XUAT_KHO:   { labelKey: "wh_filter_out",  icon: "upload",      cls: "text-blue-300 bg-[#C8A557]/15 border-[#C8A557]/30",         arrow: "↑" },
  CHUYEN_KHO: { labelKey: "wh_filter_move", icon: "swap_horiz",  cls: "text-purple-300 bg-[#C8A557]/15 border-[#C8A557]/30",   arrow: "⇄" },
};
...
export default function WarehousePage() {
  ...
  // P1b — Sub-role từ /api/auth/me (canonical, không tin cookie)
  const [vaiTroCty, setVaiTroCty] = useState<string | null>(null);
  const subRoleResolved = vaiTroCty !== null;
  const canRecord = userRole === 'admin' || (subRoleResolved && (vaiTroCty === '' || ['company_admin', 'warehouse'].includes(vaiTroCty)));
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] p-6 md:p-10 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#4A7C5C]/20 border border-[#4A7C5C]/30 flex items-center justify-center">
              <span className="material-symbols-outlined text-[#6FB585]">warehouse</span>
            </div>
            <div>
              <h1 className="text-2xl font-black text-white font-display">{t("wh_title")}</h1>
        ...
        {canRecord ? (
          <button onClick={() => setModal(true)}
            className="flex items-center gap-2 px-4 py-2.5 bg-[#C8A557]/20 hover:bg-[#C8A557]/30 text-cyan-300 border border-[#C8A557]/40 rounded-xl font-bold text-sm transition active:scale-95">
            <span className="material-symbols-outlined text-[18px]">add</span>
            {t("wh_log_tx")}
          </button>
        ) : (
          <div className="flex items-center gap-1.5 px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-xs text-slate-400">
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        {[
          { label: t("wh_total_in"),  value: stats.totalNhap.toLocaleString(), icon: "download",  color: "text-[#6FB585]" },
          { label: t("wh_total_out"), value: stats.totalXuat.toLocaleString(), icon: "upload",    color: "text-[#C8A557]" },
      ...
      <div className="flex gap-2 mb-6">
        {[["all", t("wh_filter_all")], ["NHAP_KHO", t("wh_filter_in")], ["XUAT_KHO", t("wh_filter_out")], ["CHUYEN_KHO", t("wh_filter_move")]].map(([k, l]) => (
          <button key={k} onClick={() => setFilterType(k)}
            className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition ${filterType === k ? "bg-[#C8A557]/25 text-cyan-200 border-[#C8A557]/50" : "bg-white/5 text-slate-300 border-white/10 hover:bg-white/10"}`}>
            {l}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-slate-400">
          ...
          <button onClick={() => setModal(true)}
            className="mt-2 px-5 py-2 bg-[#C8A557]/20 hover:bg-[#C8A557]/30 text-cyan-300 border border-[#C8A557]/40 rounded-xl font-bold text-sm transition active:scale-95">
            + {t("wh_log_first")}
          </button>
        </div>
      ) : (
            ...
            <table className="w-full text-left min-w-[700px]">
              <thead className="bg-white/5">
                <tr className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                  ...
                  <th className="px-5 py-3">{tr("Mã lô", "Batch code")}</th>
                ...
                {filtered.map(tx => {
                  ...
                  return (
                    <tr key={tx.id} className="hover:bg-white/5 transition">
                      ...
                      <td className="px-5 py-3">
                        <span className={`text-lg font-black ${gd.arrow === "↓" ? "text-[#6FB585]" : gd.arrow === "↑" ? "text-[#C8A557]" : "text-[#C8A557]"}`}>
      ...
      >
        <form onSubmit={handleSubmit}>
          <div className="px-6 py-5 space-y-4">
            <div className="p-3 bg-[#C8A557]/10 border border-[#C8A557]/20 rounded-xl text-xs text-blue-300 flex gap-2">
            ...
            {selectedBatch && (
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1.5">{tr("Mã lô", "Batch code")}</label>
                <div className="flex items-center gap-2 px-3 py-2.5 bg-[#C8A557]/10 border border-[#C8A557]/30 rounded-xl">
                  <span className="material-symbols-outlined text-[#C8A557] text-[18px]">qr_code</span>
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-white/5 border border-white/20 rounded-xl px-3 py-2.5 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-[#C8A557]"
              />
            </div>

            <div>
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1.5">{tr("Ghi chú", "Note")}</label>
              <textarea
                value={form.ghiChu || ""}
                onChange={e => setForm(f => ({ ...f, ghiChu: e.target.value }))}
                rows={2} placeholder="Lý do nhập/xuất, người giao nhận, xe số..."
                className="w-full bg-white/5 border border-white/20 rounded-xl px-3 py-2.5 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-[#C8A557] resize-none"
              />
            </div>
          </div>

          {/* Sticky footer */}
          <div className="flex gap-3 px-6 py-4 border-t border-white/10 bg-[#0B1623] sticky bottom-0">
            <button type="button"
            ...
            >{tr("Huỷ", "Cancel")}</button>
            <button type="submit" disabled={submitting}
              className="flex-1 py-3 bg-[#C8A557]/20 hover:bg-[#C8A557]/30 text-cyan-300 border border-[#C8A557]/40 rounded-xl text-sm font-bold transition disabled:opacity-50 flex items-center justify-center gap-2"
            >
