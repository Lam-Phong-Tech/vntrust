    ...
    <div className="min-h-screen w-full p-4 sm:p-6 lg:p-8 pb-[100px] md:pb-8">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            {tr(
              "Điều chỉnh ngưỡng AI, retention, kênh thông báo toàn hệ thống",
        ...
        <Link
          href="/dashboard"
          className="px-3 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-xs flex items-center gap-1.5"
        >
      ...
      {loading ? (
        ...
        <div className="space-y-6 max-w-4xl">
          {(Object.keys(NS_META) as Array<keyof typeof NS_META>).map((namespace) => {
            const meta = NS_META[namespace];
            const items = data[namespace as keyof ConfigData] || {};

            return (
              <div key={namespace} className="rounded-2xl bg-white/5 border border-white/10 overflow-hidden">
                      ...
                      <div key={key} className="px-5 py-3">
                        ...
                        <div className="flex gap-2">
                          ...
                          {isDirty && (
                            ...
                            >
                              {saving === key && (
                                <span className="w-3 h-3 border border-[#C8A557] border-t-transparent rounded-full animate-spin" />
                              )}
                              {tr("Lưu", "Save")}
                            </button>
                          )}
                          {item.source === "db" && !isDirty && (
                            <button
                              onClick={() => resetKey(key, namespace)}
                              disabled={saving === key}
                        ...
                        {item.ngayCapNhat && (
                          <p className="text-[9px] text-slate-500 mt-1 font-mono">
                            ...
                            {new Date(item.ngayCapNhat).toLocaleString("vi-VN")} · {item.capNhatBoi || "admin"}
                          </p>
                        )}
                      </div>
                    );
      ...
      {toast && (
        <div
          className={`fixed bottom-[90px] md:bottom-6 left-1/2 -translate-x-1/2 z-50 px-4 py-3 rounded-xl text-sm font-bold border shadow-lg backdrop-blur ${
            toast.ok
