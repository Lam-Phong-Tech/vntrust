...
// Factory functions để tái dùng được trong component với language context
type Severity = { label: string; cls: string; icon: string; pulse: boolean };
function makeMucDoMap(lang: string): Record<string, Severity> {
  const tr = (vi: string, en: string) => (lang === 'en' ? en : vi);
  return {
    high:   { label: tr("Nghiêm trọng", "Severe"),  cls: "text-red-300 bg-red-500/15 border-red-500/40",      icon: "emergency",   pulse: true  },
    medium: { label: tr("Trung bình", "Medium"),    cls: "text-amber-300 bg-[#C8A557]/15 border-[#C8A557]/40", icon: "warning",     pulse: false },
    low:    { label: tr("Thấp", "Low"),             cls: "text-blue-300 bg-[#C8A557]/15 border-[#C8A557]/30",  icon: "info",        pulse: false },
  };
}
type Status = { label: string; cls: string; icon: string };
function makeStatusMap(lang: string): Record<string, Status> {
  const tr = (vi: string, en: string) => (lang === 'en' ? en : vi);
  return {
    open:      { label: tr("Chưa xử lý", "Pending"),       cls: "text-red-300 bg-red-500/15 border-red-500/30",         icon: "radio_button_checked" },
    reviewing: { label: tr("Đang điều tra", "Investigating"), cls: "text-amber-300 bg-[#C8A557]/15 border-[#C8A557]/30",   icon: "manage_search" },
    closed:    { label: tr("Đã đóng", "Closed"),           cls: "text-emerald-300 bg-[#4A7C5C]/15 border-[#4A7C5C]/30", icon: "check_circle" },
  };
}
function makeLoaiLabels(lang: string): Record<string, string> {
  const tr = (vi: string, en: string) => (lang === 'en' ? en : vi);
  return {
    NGUOI_DUNG_BAO_CAO: tr("Báo cáo từ Người dùng", "User report"),
    AI_PHAT_HIEN:       tr("AI Phát hiện Anomaly", "AI Anomaly Detection"),
    QUET_BAN_THUONG:    tr("Quét bất thường", "Abnormal scan"),
    HANG_TRON_NOI:      tr("Hàng trôi nổi", "Drift goods"),
    MANUAL:             tr("Tạo thủ công", "Manual"),
  };
}
...
export default function AlertsPage() {
  ...
  const tr = (vi: string, en: string) => (lang === 'en' ? en : vi);
  const MUC_DO = makeMucDoMap(lang);
  const STATUS_MAP = makeStatusMap(lang);
  const LOAI_LABELS = makeLoaiLabels(lang);
  ...
  const handleUpdateStatus = async (id: string, trangThai: string, ghiChu?: string) => {
    ...
    else showToast("✗ " + tr("Cập nhật thất bại", "Update failed"), false);
    setUpdatingId(null);
  };

  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.moTa.trim()) { showToast("✗ " + tr("Vui lòng điền mô tả cảnh báo", "Please enter alert description"), false); return; }
    setSubmitting(true);
    const res = await fetch("/api/alerts", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    else { showToast("✓ " + tr("Đã tạo cảnh báo", "Alert created"), true); setCreateModal(false); setForm({ loai: "MANUAL", mucDo: "medium", moTa: "", uid: "" }); fetchData(); }
    setSubmitting(false);
  };
  ...
  const handleRunLifecycleCheck = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        showToast("✓ " + tr("Đã quét vòng đời thành công", "Lifecycle scan succeeded"), true);
        fetchData();
      } else {
        showToast("✗ " + tr("Quét thất bại", "Scan failed") + ": " + data.error, false);
      }
    } catch (e: any) {
      showToast("✗ " + tr("Lỗi kết nối", "Connection error"), false);
    }
    setLoading(false);
  };
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] p-6 md:p-10 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black text-white font-display flex items-center gap-2">
                {tr("Quản lý Cảnh báo", "Alert Management")}
                {stats.open > 0 && (
                  <span className="text-sm px-2 py-0.5 bg-red-500 text-white rounded-full animate-pulse font-bold">
                    {stats.open} {tr("mới", "new")}
                  </span>
                )}
              </h1>
              <p className="text-sm text-slate-400">{tr("BR-06: Real-time Alert System — cảnh báo hàng giả, bất thường chuỗi cung ứng", "BR-06: Real-time Alert System — counterfeit & supply chain anomaly alerts")}</p>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {userRole === "admin" && (
            <button onClick={handleRunLifecycleCheck}
              ...
              {tr("Quét Vòng Đời", "Lifecycle Scan")}
            </button>
          )}
          {userRole === "admin" && (
            <button onClick={() => setCreateModal(true)}
              ...
              {tr("Tạo cảnh báo thủ công", "Create manual alert")}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        {[
          { label: tr("Tổng cảnh báo", "Total alerts"), value: stats.total, icon: "notifications", color: "text-white" },
          { label: tr("Chưa xử lý", "Pending"),         value: stats.open,  icon: "radio_button_checked", color: "text-red-400", pulse: stats.open > 0 },
          { label: tr("Nghiêm trọng", "Severe"),         value: stats.high,  icon: "emergency",    color: "text-red-500" },
          { label: tr("Đã đóng", "Closed"),              value: stats.closed,icon: "check_circle", color: "text-[#6FB585]" },
        ].map((s, i) => (
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        <div className="flex gap-2">
          {[["all", tr("Tất cả", "All")], ["open", tr("Chưa xử lý", "Pending")], ["reviewing", tr("Điều tra", "Investigating")], ["closed", tr("Đã đóng", "Closed")]].map(([k,l]) => (
        ...
        <div className="flex gap-2">
          {[
            ["all",    tr("Tất cả mức", "All levels")],
            ["high",   tr("Nghiêm trọng", "Severe")],
            ["medium", tr("Trung bình", "Medium")],
            ["low",    tr("Thấp", "Low")],
          ].map(([k,l]) => (
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-slate-400">
          ...
          {tr("Không có cảnh báo nào", "No alerts")}
        </div>
      ) : (
                  ...
                  <div className="flex gap-3 flex-1">
                    ...
                    <div className="flex-1">
                      <div className="flex flex-wrap items-center gap-2 mb-1">
                        ...
                        {alert.uid && (
                          <span
                            className="font-mono text-xs text-cyan-300 bg-[#C8A557]/10 px-2 py-0.5 rounded-lg border border-[#C8A557]/20 max-w-[120px] truncate inline-block align-middle"
                            title={alert.uid}
                          >
                        ...
                        if (meta) {
                          return (
                            <div className="space-y-2 mt-1">
                              <p className="text-sm text-white font-bold leading-snug">{meta.lyDo || tr("Báo cáo từ người dùng", "User report")}</p>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1.5 text-xs bg-white/5 p-3 rounded-xl border border-white/10 mt-1">
                                {meta.loaiSanPham && (
                                  <p className="flex items-center gap-1.5">
                                    <span className="text-slate-400">{tr("Loại SP:", "Product type:")}</span>
                                    <span className="text-slate-200 font-medium">{meta.loaiSanPham}</span>
                                  </p>
                                )}
                                {meta.viTri && (
                                  <p className="flex items-center gap-1.5">
                                    <span className="text-slate-400">{tr("Vị trí:", "Location:")}</span>
                                    <span className="text-slate-200 font-medium">{meta.viTri}</span>
                                  </p>
                                )}
                                {meta.giaMua !== undefined && (
                                  <p className="flex items-center gap-1.5">
                                    <span className="text-slate-400">{tr("Giá mua:", "Price paid:")}</span>
                                    <span className="text-[#C8A557] font-bold">
                                      {Number(meta.giaMua).toLocaleString(lang === 'en' ? "en-US" : "vi-VN")} {meta.donViTien || "VND"}
                                    </span>
                                  </p>
                                )}
                                {meta.contactInfo && (
                                  <p className="flex items-center gap-1.5">
                                    <span className="text-slate-400">{tr("Liên hệ:", "Contact:")}</span>
                                ...
                                <div className="flex gap-2 mt-2">
                                  {meta.anhBangChung.map((img: string, i: number) => (
                                    ...
                                    >
                                      ...
                                      <img src={img} alt={tr("Bằng chứng", "Evidence")} className="w-full h-full object-cover" />
                                    </a>
                                  ))}
                                </div>
                              )}
                ...
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                   ...
                   <div className="space-y-4 relative before:absolute before:inset-0 before:ml-[5px] before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-700 before:to-transparent">
                     <div className="relative flex items-start gap-4">
                        <div className="w-3 h-3 rounded-full bg-[#4A7C5C] ring-4 ring-slate-900 z-10 shrink-0 mt-1"></div>
