...
export function BottomSheetModal({
  ...
  return (
    <div
      // pb-[80px] = chừa chỗ cho mobile bottom nav (~64px + FAB cộng thêm)
      className="fixed inset-0 z-50 flex items-end md:items-center justify-center md:p-4 pb-[88px] md:pb-4 bg-black/60 backdrop-blur-sm"
      onClick={onClose}
    >
      <div
        className={[
          ...
          // mobile: max-h trừ 168px (top safe area + bottom nav + breathing room)
          "rounded-t-3xl max-h-[calc(100dvh-168px)]",
          // desktop: centered dialog, limited width, fully rounded
          `md:rounded-3xl md:max-h-[88dvh] ${maxW}`,
        ].join(" ")}
