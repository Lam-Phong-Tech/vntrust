      ...
      <div className="relative w-full max-w-md glass-panel border border-white/20 rounded-3xl p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#C8A557] flex items-center justify-center shrink-0">
              <span className="material-symbols-outlined text-white">smart_toy</span>
            </div>
            <h2 className="text-lg font-black text-white font-display">{t("nav_chat_title")}</h2>
          </div>
        ...
        <div className="flex flex-col h-[480px]">
          ...
          <div className="flex gap-2 mt-2">
            ...
            <button onClick={send} disabled={isOverLimit || !input.trim() || typing}
              className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 transition ${isOverLimit || !input.trim() || typing ? "bg-white/20 cursor-not-allowed opacity-40" : "bg-[#C8A557] hover:bg-[#C8A557]"}`}>
      ...
      <div className="relative w-full max-w-[420px] rounded-[24px] p-6 shadow-2xl overflow-hidden"
        ...
        <div className="flex items-center justify-between mb-7">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-white text-[19px] tracking-wide font-display">{t("nav_app_title")}</span>
          </div>
            ...
            {isLoggedIn && (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-black transition border active:scale-95 ${
                  geoEnabled
                    ? "bg-[#C8A557]/20 text-cyan-300 border-[#C8A557]/40 hover:bg-[#C8A557]/30"
            ...
            {isLoggedIn && (
              <div className="relative" ref={userMenuRef}>
                <button
                  ...
                  className="w-9 h-9 rounded-full bg-gradient-to-br from-[#6FB585] to-[#E4D2A1] text-white flex items-center justify-center text-sm font-black shadow-lg hover:ring-2 hover:ring-[#C8A557]/50 transition border-2 border-[#0b1623]"
                ...
                {showUserMenu && (
                  <div className="absolute right-0 top-12 w-52 bg-[#0d1b2e] border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50">
                    ...
                    <a href="/dashboard/profile"
                      ...
                      <span className="material-symbols-outlined text-[17px] text-[#C8A557]">manage_accounts</span>
                      Hồ sơ cá nhân
                    </a>
          ...
          <div className="bg-[rgba(10,20,38,0.98)] backdrop-blur-xl border-b border-white/10 p-6 flex flex-col gap-4" onClick={e => e.stopPropagation()}>
            {navLinks.map(({ key, href }) => (
              <a key={href} href={href} onClick={() => setMobileOpen(false)}
                className={`text-base font-bold tracking-widest uppercase py-2 border-b border-white/10 ${isActive(href) ? 'text-[#C8A557]' : 'text-slate-300 hover:text-white'
                  }`}>
                {t(key)}
              </a>
            ))}
