      ...
      {open && (
        <div className="absolute right-0 top-11 w-80 sm:w-96 z-[9998] rounded-2xl shadow-2xl border border-white/10 overflow-hidden"
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
            ...
            {unread > 0 && (
              <button onClick={markAllRead} className="text-[11px] text-[#C8A557] hover:text-cyan-300 font-bold transition">
                Đọc tất cả
              </button>
            )}
          </div>
          ...
          <div className="max-h-[380px] overflow-y-auto">
            {notifications.length === 0 ? (
              ...
              <div
                ...
                className={`flex gap-3 px-4 py-3 border-b border-white/5 cursor-pointer transition hover:bg-white/5 ${!n.daDoc ? "bg-[#C8A557]/5" : ""}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {!n.daDoc && (
                      <span className="w-2 h-2 rounded-full bg-[#C8A557] shrink-0 mt-1" />
                    )}
                  </div>
