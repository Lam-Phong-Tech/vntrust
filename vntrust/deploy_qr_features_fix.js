    ...
    stream.on('close', () => {
      ...
      conn.sftp((err, sftp) => {
        ...
        const uploadNext = () => {
          ...
          const remotePath = '/var/www/vntrust/' + file.replace(/\[/g, '\\[').replace(/\]/g, '\\]');
          
          const readStream = fs.createReadStream(localPath);
          const writeStream = sftp.createWriteStream('/var/www/vntrust/' + file); // SFTP doesn't need escaping the same way as bash
