...
export default function WarehousePage() {
  ...
  // Selected batch info (find by maLo since we use maLo as value)
  const selectedBatch = batches.find(b => b.maLo === form.loHangId);
                  ...
                  return (
                    <tr key={tx.id} className="hover:bg-white/5 transition">
                      ...
                      <td className="px-5 py-3 text-right">
                        {tx.loaiGD === "NHAP_KHO" && (
                          <button 
                            onClick={() => { setForm({ loHangId: tx.loHang.maLo, loaiGD: "XUAT_KHO" }); setModal(true); }}
                            className="px-3 py-1.5 bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 rounded-lg text-xs font-bold transition flex items-center gap-1 ml-auto"
                            title="Tạo lệnh xuất kho cho lô này"
                          >
                            <span className="material-symbols-outlined text-[14px]">upload</span> Xuất kho
                          </button>
                        )}
                      </td>
                    </tr>
