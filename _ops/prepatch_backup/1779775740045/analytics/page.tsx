              ...
              {scanStats.scanTrend.map((day, i) => {
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    ...
                    <span className="text-[9px] text-slate-500 whitespace-nowrap">{day.date.split("/").slice(0, 2).join("/")}</span>
                  </div>
                );
              })}
            </div>
            ...
            {expiringData.expiring.length === 0 ? (
              ...
              <div className="space-y-2">
                {expiringData.expiring.map(lo => {
                  ...
                  return (
                    <div key={lo.id} className={`flex items-center justify-between p-4 rounded-xl border ${daysLeft <= 7 ? "bg-red-500/10 border-red-500/30" : "bg-amber-500/10 border-amber-500/20"}`}>
                      <div>
            ...
            <div className="glass-panel border border-red-500/20 rounded-2xl p-5">
              ...
              <div className="space-y-2">
                {expiringData.expired.slice(0, 10).map(lo => (
                  <div key={lo.id} className="flex items-center justify-between p-3 rounded-xl bg-red-500/10 border border-red-500/20">
                    <div>
