      ...
      <main className="mx-auto max-w-7xl w-full flex-1 p-8 lg:p-12 min-h-[calc(100vh-80px)] overflow-x-hidden">
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-10 gap-4">
          <div>
            ...
            <h2 className="font-headline text-3xl sm:text-4xl font-extrabold text-white">{t("hist_title")}</h2>
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <button onClick={() => setFilter("warning")}
              className={`px-3 py-1.5 text-xs font-bold rounded-xl transition ${filter === "warning" ? "bg-amber-500 text-white" : "bg-white/5 glass-panel text-slate-300 border border-white/20 hover:bg-white/10"}`}>
              {t("hist_warning")}
            </button>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-white/10">
                      <th className="pb-4">{t("hist_col_action")}</th>
                      <th className="pb-4">{t("hist_col_user")}</th>
                      {userRole === "admin" && <th className="pb-4">{t("hist_col_role")}</th>}
                      <th className="pb-4">{t("hist_col_ip")}</th>
                      <th className="pb-4">{t("hist_col_time")}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/5">
                    {filtered.map(log => (
                      <tr key={log.id} className="hover:bg-white/5 transition">
                        ...
                        <td className="py-4 text-sm text-white font-bold">{log.user}</td>
                        {userRole === "admin" && (
                          <td className="py-4">
                        ...
                        <td className="py-4 text-xs font-semibold text-slate-400">{formatTime(log.time)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                  ...
                  <div key={log.id} className="bg-white/5 rounded-2xl p-4 border border-white/10">
                    <div className="flex items-start gap-3 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-slate-200 font-medium leading-snug">{log.action}</p>
                        <p className="text-xs text-white font-bold mt-1">{log.user}</p>
                      </div>
