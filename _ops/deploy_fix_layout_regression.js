...
async function main() {
  ...
  const backupDir = `${REMOTE_ROOT}/_backups/fix_regression.${stamp}`;
  await run(conn, `mkdir -p ${backupDir}`);
  for (const rel of FILES) {
    const bkp = `${backupDir}/${rel.replace(/[/\\]/g, '_')}`;
    await run(conn, `cp '${REMOTE_ROOT}/${rel}' '${bkp}'`);
  }
  ...
  console.log('\n[verify] data-frame-mode tất cả role giờ là responsive:');
  for (const role of ['', 'consumer', 'admin', 'manufacturer', 'importer']) {
    const cookie = role ? `userRole=${role};userName=Test` : '';
    r = await run(conn, `curl -sk -H "Cookie: ${cookie}" "https://anticounterfeit.test9.io.vn/dashboard" | grep -oE 'data-frame-mode="[a-z]+"' | head -1`);
    console.log(`  role="${role || 'anonymous'}":  ${r.stdout.trim() || '(redirect — not authenticated)'}`);
  }
