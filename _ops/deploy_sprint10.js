...
async function main() {
  ...
  const backupDir = `${REMOTE_ROOT}/_backups/sprint10.${stamp}`;
  ...
  // Install cron entry
  console.log('[cron] adding anomaly-detection entry...');
  const cronCheck = await run(conn, 'crontab -l 2>/dev/null | grep "anomaly-detection" || echo "MISSING"');
  if (cronCheck.stdout.includes('MISSING')) {
    await run(conn, `(crontab -l 2>/dev/null; echo '*/30 * * * * curl -s "http://localhost:3000/api/cron/anomaly-detection?secret=vntrust-cron-key" >/dev/null') | crontab -`);
    console.log('[cron] installed: anomaly-detection every 30 min');
  } else {
    console.log('[cron] already present, skipping');
  }
