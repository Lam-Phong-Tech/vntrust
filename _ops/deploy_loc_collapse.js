// Make "Vị trí xác thực lần cuối" card collapsible
...
async function main(){
  ...
  const bk=`${REMOTE}/_backups/loc_collapse.${Date.now()}`;
  ...
  for(const r of FILES){await new Promise((rs,j)=>sftp.fastPut(path.join(LOCAL,r),`${REMOTE}/${r}`,e=>e?j(e):rs()));console.log('✓',r);}
  console.log('[build]...');
  ...
  c.end();console.log(`✅ Done. Backup: ${bk}`);
}
