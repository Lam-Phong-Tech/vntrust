...
const FILES = [
  ...
  'src/app/login/select-role/page.tsx',
...
async function main() {
  ...
  const backupDir = `${REMOTE_ROOT}/_backups/4fixes.${stamp}`;
  ...
  console.log('\n[verify]');
  // /login should redirect to /login/consumer (307 or 308)
  r = await run(conn, 'curl -sk -o /dev/null -w "/login STATUS=%{http_code} LOC=%{redirect_url}" --max-redirs 0 "https://anticounterfeit.test9.io.vn/login"');
  console.log(r.stdout);
  // /login/select-role accessible (200)
  r = await run(conn, 'curl -sk -o /dev/null -w "\\n/login/select-role STATUS=%{http_code}" "https://anticounterfeit.test9.io.vn/login/select-role"');
  console.log(r.stdout);
  // /forgot-password should have gold button now
  r = await run(conn, 'curl -sk "https://anticounterfeit.test9.io.vn/forgot-password" | grep -oE "bg-\\[#C8A557\\]|bg-\\[#3cdada\\]" | sort | uniq -c');
  console.log('Forgot-password button colors:'); console.log(r.stdout);
