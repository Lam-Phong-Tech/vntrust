// Sprint 14 — Mobile bottom nav redesign + mobile dashboard live stats + admin user mgmt
...
async function main() {
  ...
  for (const rel of FILES) {
    if (!fs.existsSync(path.join(LOCAL_VNTRUST, rel))) {
      console.error(`[ERR] Missing local file: ${rel}`); process.exit(1);
    }
  }
  ...
  const backupDir = `${REMOTE_ROOT}/_backups/admin_users.${stamp}`;
