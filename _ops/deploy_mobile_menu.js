// Deploy mobile menu drawer + logo fix
...
const FILES = [
  ...
  'src/components/MobileMenuDrawer.tsx',  // new file
];
...
async function main() {
  ...
  const backupDir = `${REMOTE_ROOT}/_backups/mobile_menu.${stamp}`;
  await run(conn, `mkdir -p ${backupDir}`);
  for (const rel of FILES) {
    const remote = `${REMOTE_ROOT}/${rel}`;
    const bkp = `${backupDir}/${rel.replace(/[/\\]/g, '_')}`;
    await run(conn, `[ -f '${remote}' ] && cp '${remote}' '${bkp}' || echo "(new file, skip backup)"`);
  }
