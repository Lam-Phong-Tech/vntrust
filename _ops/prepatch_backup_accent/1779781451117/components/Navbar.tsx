            ...
            <div className="hidden lg:flex items-center gap-7 pt-0.5">
              {navLinks.map(({ key, href }) => (
                <Link key={href} href={href}
                  className={`text-[13px] font-bold font-display tracking-widest transition-all uppercase pb-1 ${isActive(href) ? "text-white border-b-2 border-cyan-400 glow-text" : "text-blue-100/70 hover:text-white"}`}>
                  {t(key)}
                </Link>
              ))}
            </div>
