...
async function main() {
  ...
  const backupDir = `${REMOTE_ROOT}/_backups/role_layout.${stamp}`;
  ...
  // Verify: 4 role × layout mode
  console.log('\n[verify] data-frame-mode attribute per role:');
  for (const role of ['', 'consumer', 'admin', 'manufacturer', 'importer']) {
    ...
    console.log(`  role="${role || 'anonymous'}":  ${r.stdout.trim() || '(no frame attr — maybe redirect)'}`);
  }
