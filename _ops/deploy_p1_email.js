// Deploy B — P1 (API sub-role guards + UI guards) + Email service
...
async function main() {
  ...
  const backupDir = `${REMOTE_ROOT}/_backups/p1_email.${stamp}`;
  ...
  // Pre-create all parent directories before fastPut (avoids "No such file" race)
  for (const rel of FILES) {
