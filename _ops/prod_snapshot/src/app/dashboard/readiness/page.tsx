...
const UC_STATUS: Record<string, { icon: string; cls: string; label: string }> = {
  ...
  partial: { icon: "clock_loader_40", cls: "text-amber-400", label: "Một phần" },
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-amber-500/20 border border-amber-500/30 flex items-center justify-center">
              <span className="material-symbols-outlined text-amber-400">rocket_launch</span>
            </div>
            <div>
              <h1 className="text-2xl font-black text-white font-headline">Release Readiness</h1>
              <p className="text-sm text-slate-400">Phần 13: Tiêu chí chấp nhận · Phần 12.2: Decision Gates · 27 Use Cases</p>
            </div>
          </div>
          {/* Overall progress */}
          <div className="flex items-center gap-3 glass-panel border border-amber-500/20 rounded-xl px-4 py-2">
      ...
      <div className="flex gap-2 mb-6">
        {[["gate","Decision Gates","flag"],["ac","Acceptance Criteria","checklist"],["uc","Use Case Tracker","task_alt"]].map(([k,l,icon]) => (
          <button key={k} onClick={() => setActiveTab(k as any)}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-bold border transition ${activeTab === k ? "bg-amber-500 text-white border-amber-400" : "bg-white/5 text-slate-300 border-white/10 hover:bg-white/10"}`}>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${gate.status === "in_progress" ? "bg-amber-500 text-white" : gate.status === "done" ? "bg-emerald-500 text-white" : "bg-white/10 text-slate-400"}`}>
                  ...
                  <div className="text-right">
                    <p className={`text-3xl font-black ${pct === 100 ? "text-emerald-400" : pct >= 50 ? "text-amber-400" : "text-red-400"}`}>{pct}%</p>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {["all","NSX","NPP","NTD","ADM"].map(a => (
              <button key={a} onClick={() => setActorFilter(a)}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition ${actorFilter === a ? "bg-amber-500 text-white border-amber-400" : "bg-white/5 text-slate-300 border-white/10 hover:bg-white/10"}`}>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              { label: "Một phần",   count: partialCount, cls: "text-amber-400 bg-amber-500/10 border-amber-500/20" },
