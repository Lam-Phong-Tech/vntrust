    ...
    <div className="min-h-[calc(100vh-80px)] p-6 md:p-10 max-w-7xl mx-auto">
      ...
      {investigateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setInvestigateModal(null)}>
          <div className="bg-[#0f1e33] border border-amber-500/30 rounded-3xl p-7 w-full max-w-4xl shadow-2xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-6 shrink-0">
              <div className="w-12 h-12 rounded-xl bg-amber-500/20 border border-amber-500/30 flex items-center justify-center">
                <span className="material-symbols-outlined text-[24px] text-amber-400">manage_search</span>
              </div>
              <div>
                        ...
                        return (
                          <div className="space-y-2 text-sm text-slate-300">
                            ...
                            <div className="grid grid-cols-2 gap-4 text-xs bg-slate-900/40 p-3 rounded-lg border border-slate-700/50">
                              ...
                              {meta.giaMua !== undefined && <p><span className="text-slate-400">Giá mua:</span> <span className="text-amber-400 font-bold">{Number(meta.giaMua).toLocaleString('vi-VN')} {meta.donViTien || 'VND'}</span></p>}
                            ...
                            {meta.anhBangChung && Array.isArray(meta.anhBangChung) && meta.anhBangChung.length > 0 && (
                              <div className="mt-2">
                                ...
                                <div className="flex gap-2">
                                  {meta.anhBangChung.map((img: string, i: number) => (
                                    <a key={i} href={img} target="_blank" rel="noreferrer" className="w-16 h-16 bg-black/30 rounded-lg flex items-center justify-center border border-white/10 hover:border-amber-400 overflow-hidden shrink-0 transition">
                                      {/* eslint-disable-next-line @next/next/no-img-element */}
                                      <img src={img} alt="Bằng chứng" className="w-full h-full object-cover" />
                                    </a>
                                  ))}
              ...
              <div className="flex-1 space-y-4">
                ...
                <div className="space-y-3">
                  <textarea
                    ...
                    className="w-full bg-slate-900/50 border border-white/20 rounded-xl px-4 py-3 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-amber-400 resize-none"
                  />
            ...
            <div className="mt-6 pt-5 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4 shrink-0">
               ...
               <div className="flex w-full sm:w-auto gap-3">
                  ...
                  <button type="button" disabled={updatingId === investigateModal.id || !investigateNote.trim()}
                    ...
                    className="flex-1 sm:flex-none px-6 py-2.5 bg-amber-500 hover:bg-amber-400 text-white rounded-xl text-sm font-bold transition disabled:opacity-50 flex items-center justify-center gap-2">
