...
const FILES = [
  ...
  // Shopee integration
  'src/lib/shopeeIntegration.ts',
  'src/app/api/shopee/route.ts',
  // Real AI verify (no GPU)
...
async function main() {
  ...
  const backupDir = `${REMOTE_ROOT}/_backups/shopee_ai.${stamp}`;
  await run(conn, `mkdir -p ${backupDir}`);
  for (const rel of FILES) await run(conn, `cp '${REMOTE_ROOT}/${rel}' '${backupDir}/${rel.replace(/[/\\]/g, '_')}' || true`);
