...
const FILES = [
  // i18n consumer dashboard
  'src/app/dashboard/page.tsx',
  // Integration Hub purpose + i18n
  'src/app/dashboard/integration/page.tsx',
  'src/app/api/gov-integration/route.ts',
  // B2: key versioning lib
  'src/lib/vaultCrypto.ts',
  // B1 cron: anonymous session purge
  'src/app/api/cron/purge-sessions/route.ts',
  // B2 cron: AES key rotation alert
  'src/app/api/cron/key-rotation-check/route.ts',
];
...
async function main() {
  ...
  const backupDir = `${REMOTE_ROOT}/_backups/sprint3.${stamp}`;
  ...
  // Install crontab if not present
  console.log('[cron] checking crontab...');
  const cronCheck = await run(conn, 'crontab -l 2>/dev/null | grep -E "purge-sessions|key-rotation-check" || echo "NOT_INSTALLED"');
  if (cronCheck.stdout.includes('NOT_INSTALLED') || !cronCheck.stdout.includes('purge-sessions')) {
    console.log('[cron] installing entries...');
    await run(conn, `(crontab -l 2>/dev/null; echo '0 3 * * * curl -s "http://localhost:3000/api/cron/purge-sessions?secret=vntrust-cron-key" >/dev/null'; echo '0 4 * * 1 curl -s "http://localhost:3000/api/cron/key-rotation-check?secret=vntrust-cron-key" >/dev/null') | crontab -`);
    console.log('[cron] installed: purge-sessions @ 03:00 daily, key-rotation-check @ 04:00 Mon');
  } else {
    console.log('[cron] entries already present, skipping');
  }
