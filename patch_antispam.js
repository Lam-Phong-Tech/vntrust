...
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    if (err) throw err;
    console.log('Uploading send-otp/route.ts...');
    sftp.fastPut(
      path.join('D:/Web hang gia/vntrust', 'src/app/api/auth/send-otp/route.ts'),
      '/var/www/vntrust/src/app/api/auth/send-otp/route.ts',
      (err) => {
