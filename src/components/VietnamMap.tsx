...
export default function VietnamMap({
  ...
  const labelFontSize = Math.max(0.5, 4.8 / position.zoom);
  const markerFontSize = 4.2 / position.zoom;
    ...
    >
      ...
      >
        ...
        >
          ...
          {ISLANDS.map((island) => (
            <g key={island.name}>
              <Marker coordinates={island.center}>
                ...
                <text textAnchor="middle" y={-20} fontSize={3.5} fontWeight="bold"
          ...
          {/* ── Province name labels (skip marker cities) ── */}
          {ALL_PROVINCES
      ...
      {hoveredMarker && (
        ...
        >
          <div
            className="rounded-2xl border border-cyan-400/40 shadow-2xl backdrop-blur-md p-3 text-sm"
          ...
          >
            ...
            {hoveredMarker.fake > 0 ? (
              ...
              <p className="text-emerald-400 text-xs mt-0.5">✓ An toàn</p>
            )}
            {hoveredMarker.scans > 0 && (
          ...
          {/* Arrow */}
          <div
            className="absolute w-2 h-2 bg-[rgba(2,12,28,0.95)] border-r border-b border-cyan-400/40 rotate-45"
