...
const TRANG_THAI_KEYS: Record<string, { labelKey: string; icon: string; cls: string }> = {
  active:              { labelKey: "dist_ready",        icon: "inventory",        cls: "text-blue-300 bg-blue-500/15 border-blue-500/30" },
  distributed:         { labelKey: "dist_shipped",      icon: "local_shipping",   cls: "text-emerald-300 bg-emerald-500/15 border-emerald-500/30" },
  suspended:           { labelKey: "dist_locked",       icon: "lock",             cls: "text-red-400 bg-red-500/15 border-red-500/30" },
  pending_review:      { labelKey: "dist_pending_admin",icon: "hourglass_top",    cls: "text-amber-300 bg-amber-500/15 border-amber-500/30" },
  pending_distributor: { labelKey: "dist_pending_dist", icon: "pending",          cls: "text-purple-300 bg-purple-500/15 border-purple-500/30" },
  ready:               { labelKey: "dist_received",     icon: "check_circle",     cls: "text-cyan-300 bg-cyan-500/15 border-cyan-500/30" },
  confirmed:           { labelKey: "dist_confirmed",    icon: "thumb_up",         cls: "text-emerald-300 bg-emerald-500/15 border-emerald-500/30" },
    ...
    <div className="min-h-[calc(100vh-80px)] p-6 md:p-10 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black text-white font-headline">{t("dist_title")}</h1>
          ...
          {["all", "active", "distributed", "suspended"].map(s => (
            <button key={s} onClick={() => { setFilterStatus(s); setPageBatch(1); }}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition border ${
                filterStatus === s ? "bg-cyan-500 text-white border-cyan-400" : "bg-white/5 text-slate-300 border-white/10 hover:bg-white/10"
              }`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        {[
          ...
          { label: t("dist_ready"),   value: batches.filter(b => b.trangThai === "active" || b.trangThai === "approved").length, icon: "inventory", color: "text-blue-400" },
          { label: t("dist_shipped"), value: batches.filter(b => b.trangThai === "distributed").length, icon: "local_shipping", color: "text-emerald-400" },
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-slate-400">
          ...
          Chưa có lô hàng nào trong danh sách phân phối. Vào <Link href="/dashboard/warehouse" className="text-cyan-400 underline">Kho hàng</Link> để nhập lô và kích hoạt xuất kho.
        </div>
      ) : (
        <div className="flex flex-col gap-4">
                  ...
                  <div className="flex gap-2 shrink-0">
                    ...
                    {batch.trangThai === "active" && userRole === "manufacturer" && (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-2 bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 rounded-xl text-xs font-bold hover:bg-cyan-500/30 transition"
                      >
                    ...
                    {batch.trangThai === "distributed" || ["pending_review","pending_distributor","ready"].includes(batch.trangThai) ? null : (
                      batch.trangThai !== "active" ? (
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-2 bg-blue-500/20 text-blue-300 border border-blue-500/30 rounded-xl text-xs font-bold hover:bg-blue-500/30 transition"
                        >
          ...
          {filtered.length > ITEMS_PER_PAGE && (
            <div className="flex justify-center items-center gap-4 mt-6">
              <button onClick={() => setPageBatch(p => Math.max(1, p - 1))} disabled={pageBatch === 1} className="px-4 py-2 bg-white/5 rounded-xl text-white disabled:opacity-30 border border-white/10">Trước</button>
      ...
      {(orders.length > 0 || userRole === 'importer') && (
        <div className="mt-10">
          ...
          <div className="flex flex-col gap-4">
            {orders.length === 0 ? (
              ...
              const SC: Record<string,string> = {
                pending_review: "text-amber-300 bg-amber-500/15 border-amber-500/30",
                pending_distributor: "text-purple-300 bg-purple-500/15 border-purple-500/30",
                confirmed: "text-emerald-300 bg-emerald-500/15 border-emerald-500/30",
                ready: "text-cyan-300 bg-cyan-500/15 border-cyan-500/30",
                distributed: "text-teal-300 bg-teal-500/15 border-teal-500/30",
              ...
              return (
                <div key={ord.id} className="glass-panel border border-white/10 rounded-2xl p-5 hover:border-white/20 transition">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                    ...
                    {canAct && (
                      <button onClick={() => setViewOrder(ord)} className="flex items-center gap-1.5 px-4 py-2 bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 rounded-xl text-xs font-bold hover:bg-cyan-500/30 transition shrink-0">
          ...
          {orders.length > ITEMS_PER_PAGE && (
            <div className="flex justify-center items-center gap-4 mt-6">
              <button onClick={() => setPageOrder(p => Math.max(1, p - 1))} disabled={pageOrder === 1} className="px-4 py-2 bg-white/5 rounded-xl text-white disabled:opacity-30 border border-white/10">Trước</button>
            ...
            {newStatus === "distributed" && (
              <div className="mb-5">
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/20 rounded-xl px-4 py-3 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-cyan-400"
                />
              </div>
            )}
            <div className="flex gap-3">
              <button onClick={() => setActionBatch(null)} className="flex-1 py-3 border border-white/20 rounded-xl text-sm font-bold text-slate-300 hover:bg-white/5 transition">Huỷ</button>
              <button onClick={handleActivate} disabled={submitting}
                className={`flex-1 py-3 rounded-xl text-sm font-bold transition disabled:opacity-50 flex items-center justify-center gap-2 ${
                  newStatus === "distributed" ? "bg-emerald-500 hover:bg-emerald-400 text-white"
                    : newStatus === "active" ? "bg-blue-500 hover:bg-blue-400 text-white"
      ...
      {transferBatch && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setTransferBatch(null)}>
          <div className="bg-[#0a1628] border border-cyan-500/20 rounded-3xl p-7 w-full max-w-xl shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-5">
              <div className="w-11 h-11 rounded-xl bg-cyan-500/20 border border-cyan-500/30 flex items-center justify-center">
                <span className="material-symbols-outlined text-cyan-400">send</span>
              </div>
              <div>
            ...
            <div className="glass-panel border border-amber-500/20 rounded-xl p-3 flex gap-2 mb-4">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-white/5 border border-white/20 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-cyan-400"
                >
              ...
              <div>
                ...
                {previewImg ? (
                  ...
                  <button onClick={() => fileInputRef.current?.click()} disabled={uploadingImg}
                    className="w-full border-2 border-dashed border-white/20 rounded-xl py-6 flex flex-col items-center gap-2 text-slate-400 hover:border-cyan-400/40 hover:text-cyan-300 transition disabled:opacity-50">
                    {uploadingImg ? <span className="animate-spin rounded-full h-5 w-5 border-t-2 border-b-2 border-cyan-400" /> : <span className="material-symbols-outlined text-3xl">upload</span>}
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => { setTransferBatch(null); setPreviewImg(""); }} className="flex-1 py-3 border border-white/20 rounded-xl text-sm font-bold text-slate-300 hover:bg-white/5 transition">Huỷ</button>
              <button onClick={handleTransfer} disabled={submitting || uploadingImg}
                className="flex-1 py-3 bg-gradient-to-r from-cyan-500 to-cyan-400 hover:from-cyan-400 hover:to-cyan-300 text-white rounded-xl text-sm font-bold transition disabled:opacity-50 flex items-center justify-center gap-2">
                {submitting && <span className="animate-spin rounded-full h-4 w-4 border-t-2 border-b-2 border-white" />}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setDeleteBatch(null)}>
          <div className="bg-[#1a2235] border border-red-500/30 rounded-3xl shadow-2xl w-full max-w-sm p-8" onClick={e => e.stopPropagation()}>
            <div className="flex flex-col items-center text-center mb-6">
              ...
              <h2 className="text-xl font-bold text-white">Xác nhận xóa lô hàng</h2>
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteBatch(null)}
                className="flex-1 py-3 border border-white/20 rounded-xl text-sm font-bold text-slate-300 hover:bg-white/5 transition">Hủy</button>
      ...
      {viewOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setViewOrder(null)}>
          <div className="bg-[#0a1628] border border-white/10 rounded-3xl p-7 w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            {/* Header */}
            <div className="flex items-center gap-3 mb-5">
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center border ${
                viewOrder.trangThai === 'pending_review' ? 'bg-amber-500/20 border-amber-500/30' :
                viewOrder.trangThai === 'pending_distributor' ? 'bg-purple-500/20 border-purple-500/30' :
                viewOrder.trangThai === 'confirmed' ? 'bg-cyan-500/20 border-cyan-500/30' :
                viewOrder.trangThai === 'distributed' ? 'bg-emerald-500/20 border-emerald-500/30' :
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-4 space-y-3 text-sm mb-5">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold border ${
                  viewOrder.trangThai === 'pending_review' ? 'text-amber-300 bg-amber-500/15 border-amber-500/30' :
                  viewOrder.trangThai === 'pending_distributor' ? 'text-purple-300 bg-purple-500/15 border-purple-500/30' :
                  viewOrder.trangThai === 'confirmed' ? 'text-cyan-300 bg-cyan-500/15 border-cyan-500/30' :
                  viewOrder.trangThai === 'distributed' ? 'text-emerald-300 bg-emerald-500/15 border-emerald-500/30' :
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {userRole === "admin" && viewOrder.trangThai === "pending_review" && (<>
                <button onClick={() => {
                  ...
                  className="flex-[2] py-3 bg-emerald-500 hover:bg-emerald-400 text-white rounded-xl text-sm font-bold transition disabled:opacity-50 flex items-center justify-center gap-2">
              ...
              {userRole === "importer" && viewOrder.trangThai === "pending_distributor" && (<>
                <button onClick={() => handleOrderAction(viewOrder.id, "confirm_shipment")} disabled={submitting}
                  className="flex-[2] py-3 bg-cyan-500 hover:bg-cyan-400 text-white rounded-xl text-sm font-bold transition disabled:opacity-50 flex items-center justify-center gap-2">
              ...
              {userRole === "importer" && viewOrder.trangThai === "ready" && (
                <button onClick={() => handleOrderAction(viewOrder.id, "delivered")} disabled={submitting}
                  className="flex-[2] py-3 bg-emerald-500 hover:bg-emerald-400 text-white rounded-xl text-sm font-bold transition disabled:opacity-50">
                  ✓ Đã giao hàng
                </button>
              )}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => { setRejectTarget(null); setRejectReason(""); }}>
          <div className="bg-[#1a0a0a] border border-red-500/30 rounded-3xl p-7 w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="mb-5">
              ...
              />
              {!rejectReason.trim() && (
                <p className="text-xs text-red-400 mt-1">Vui lòng nhập lý do từ chối</p>
              )}
            </div>
            <div className="flex gap-3">
