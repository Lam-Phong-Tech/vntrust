...
const PHASE_COLORS: Record<string, { text: string; border: string; bg: string; badge: string }> = {
  cyan:    { text: "text-cyan-400",    border: "border-cyan-500/30",    bg: "bg-cyan-500/10",    badge: "bg-cyan-500" },
  purple:  { text: "text-purple-400",  border: "border-purple-500/30",  bg: "bg-purple-500/10",  badge: "bg-purple-500" },
  emerald: { text: "text-emerald-400", border: "border-emerald-500/30", bg: "bg-emerald-500/10", badge: "bg-emerald-500" },
  amber:   { text: "text-amber-400",   border: "border-amber-500/30",   bg: "bg-amber-500/10",   badge: "bg-amber-500" },
};

const TIMELINE_STYLE: Record<string, { dot: string; textCls: string }> = {
  done:      { dot: "bg-emerald-400", textCls: "text-emerald-300" },
  gate:      { dot: "bg-cyan-400 ring-2 ring-cyan-400/40 ring-offset-1 ring-offset-transparent", textCls: "text-cyan-300 font-bold" },
  milestone: { dot: "bg-white",       textCls: "text-white font-bold" },
  pending:   { dot: "bg-slate-500",   textCls: "text-slate-400" },
  future:    { dot: "bg-amber-500",   textCls: "text-amber-300" },
};
    ...
    <div className="min-h-[calc(100vh-80px)] p-6 md:p-10 max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black text-white font-headline">Lộ trình Triển khai</h1>
          ...
          <div className="glass-panel border border-purple-500/20 rounded-xl px-4 py-2 flex items-center gap-3">
            <div>
              <div className="w-32 bg-white/10 rounded-full h-2 mb-1">
                <div className="h-2 rounded-full bg-gradient-to-r from-cyan-500 to-purple-500" style={{ width: `${pct}%` }} />
              </div>
      ...
      <div className="glass-panel border border-white/10 rounded-2xl p-5 mb-8">
        <h2 className="text-sm font-bold text-white mb-4 flex items-center gap-2">
          <span className="material-symbols-outlined text-purple-400 text-[18px]">timeline</span>
            ...
            {TIMELINE_EVENTS.map((ev, i) => {
              ...
              return (
                <div key={i} className="relative flex items-center gap-4">
                  ...
                  {ev.type === "gate" && (
                    <span className="text-[9px] px-2 py-0.5 bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 rounded-full font-bold">GATE</span>
                  )}
                </div>
              );
            })}
            ...
            <button key={ph.id} onClick={() => setSelectedPhase(selectedPhase === ph.id ? null : ph.id)}
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`text-xs font-bold flex items-center gap-1 ${
                  ph.status === "in_progress" ? "text-emerald-400" :
                  ph.status === "completed" ? "text-cyan-400" : "text-slate-500"
                }`}>
        ...
        return (
          <div className={`glass-panel border rounded-2xl p-6 ${c.border} ${c.bg}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {ph.features.map((f, i) => (
                <div key={i} className={`flex items-center gap-2 p-2.5 rounded-xl text-sm ${f.done ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-white/5 border border-white/10"}`}>
                  <span className={`material-symbols-outlined text-[16px] shrink-0 ${f.done ? "text-emerald-400" : "text-slate-500"}`}>
      ...
      {!selectedPhase && currentPhase && (
        <div className="glass-panel border border-cyan-500/20 bg-cyan-500/5 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            {currentPhase.features.map((f, i) => (
              <div key={i} className={`flex items-center gap-2 p-2 rounded-lg text-xs ${f.done ? "text-emerald-300" : "text-slate-500"}`}>
                <span className={`material-symbols-outlined text-[14px] ${f.done ? "text-emerald-400" : "text-slate-600"}`}>
                  {f.done ? "check_circle" : "radio_button_unchecked"}
                </span>
                {f.text}
              </div>
