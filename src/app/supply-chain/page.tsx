...
export default function SupplyChainPage() {
  ...
  const [chartRawData, setChartRawData] = useState<number[]>(Array(24).fill(0));
  ...
  return (
    <div className="flex transparent font-body">
      <main className="mx-auto max-w-7xl w-full flex-1 p-4 sm:p-8 lg:p-12 overflow-x-hidden min-h-[calc(100vh-80px)] transparent">
        ...
        <div className="space-y-8">
          ...
          <section className="grid grid-cols-12 gap-6">
            ...
            <div className="col-span-12 lg:col-span-8 bg-white/5 glass-panel text-white p-8 rounded-2xl shadow-sm border border-outline-variant/10 transition-all overflow-hidden">
              ...
              <div className="relative w-full h-[200px]">
                <svg viewBox="0 0 800 200" className="w-full h-full" preserveAspectRatio="none">
                  ...
                  {linePath && (
                    <>
                      ...
                      {chartPoints.length > 0 && chartPoints.map((pt, i) => {
                        ...
                        return (
                          <g key={i} onMouseEnter={() => setHoverIndex(i)} onMouseLeave={() => setHoverIndex(null)} className="cursor-pointer">
                            <rect x={x - step/2} y="0" width={step} height="200" fill="transparent" />
                            {isHovered && (
                              <>
                                <line x1={x} y1="0" x2={x} y2="200" stroke="#3cdada" strokeWidth="1" strokeDasharray="4 4" opacity="0.5" />
                                <circle cx={x} cy={pt} r="6" fill="#0f1e33" stroke="#3cdada" strokeWidth="2.5" className="animate-pulse" />
                              </>
                            )}
                          </g>
                        )
                ...
                {hoverIndex !== null && (
                  <div 
                    className="absolute z-10 bg-slate-900 border border-primary/30 px-4 py-2.5 rounded-xl shadow-[0_0_15px_rgba(60,218,218,0.2)] pointer-events-none transform -translate-x-1/2 -translate-y-full flex flex-col gap-1 items-center min-w-[120px] transition-all duration-200"
                    style={{
                      left: `${(hoverIndex / (chartPoints.length - 1)) * 100}%`,
                      top: `${chartPoints[hoverIndex]}px`,
                      marginTop: '-16px'
                    }}
                  >
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{chartLabels[hoverIndex] || "LIVE"}</p>
                    <p className="text-base font-black text-white flex items-center gap-1">
                      ...
                      {chartRawData[hoverIndex]?.toLocaleString() || 0} <span className="text-xs font-medium text-slate-400">lượt</span>
                    </p>
                    <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-3 h-3 bg-slate-900 border-r border-b border-primary/30 rotate-45"></div>
                  </div>
                )}
          ...
          <section className="bg-white/5 glass-panel text-white p-8 rounded-2xl shadow-sm border border-outline-variant/10 flex flex-col relative overflow-hidden">
            ...
            <button
              ...
              className="w-full mt-6 py-3 text-xs font-bold text-primary uppercase tracking-widest hover:bg-white/10 transition-colors rounded-lg border border-primary/20 bg-white/5 shadow-sm flex items-center justify-center gap-2"
            >
        ...
        {/* Modal Mạng lưới Blockchain */}
        {showNetworkModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowNetworkModal(false)}>
            <div className="bg-[#0f1e33] border border-primary/30 rounded-3xl p-7 w-full max-w-4xl shadow-2xl max-h-[90vh] flex flex-col relative overflow-hidden" onClick={e => e.stopPropagation()}>
              {/* Background Glow */}
              <div className="absolute top-[-50px] left-1/2 -translate-x-1/2 w-96 h-96 bg-primary/20 rounded-full blur-[100px] pointer-events-none"></div>
              
              <div className="flex items-center justify-between mb-6 relative z-10">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-primary/30 to-primary/5 border border-primary/40 flex items-center justify-center shadow-[0_0_15px_rgba(60,218,218,0.2)]">
                    <span className="material-symbols-outlined text-primary text-2xl">lan</span>
                  </div>
                  <div>
                    <h3 className="text-2xl font-black text-white font-headline tracking-tight">Trình Khám Phá Khối (Block Explorer)</h3>
                    <p className="text-sm text-primary/80 font-medium flex items-center gap-2">
                      <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
                      VNTRUST Live Network
                    </p>
                  </div>
                </div>
                <button onClick={() => setShowNetworkModal(false)} className="w-10 h-10 bg-white/5 hover:bg-red-500/20 hover:text-red-400 border border-white/10 rounded-xl flex items-center justify-center text-slate-400 transition-all">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-3 relative z-10">
                {Array.from({length: 20}).map((_, i) => {
                  const blockHeight = 1045230 - i;
                  const hash = "0x" + Array(4).fill(0).map(() => Math.random().toString(16).substr(2, 16)).join('').padEnd(64, '0');
                  const time = new Date(Date.now() - i * 15000).toLocaleTimeString("vi-VN");
                  const txCount = Math.floor(Math.random() * 50) + 1;
                  return (
                    <div key={i} className="glass-panel bg-white/[0.02] border border-white/10 rounded-2xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-primary/40 hover:bg-white/[0.04] transition-all duration-300 group">
                      <div className="flex items-center gap-4">
                        <div className="bg-black/20 border border-white/5 rounded-xl p-3 text-center min-w-[80px] group-hover:bg-primary/10 transition-colors">
                          <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1 group-hover:text-primary/70">Block</p>
                          <p className="text-lg font-mono font-black text-white">#{blockHeight}</p>
                        </div>
                        <div>
                          <div className="flex items-center gap-2 mb-2">
                            <span className="text-xs text-slate-400 font-mono hidden md:inline-block truncate max-w-[200px] lg:max-w-[400px]">Hash: {hash}</span>
                            <span className="text-xs text-slate-400 font-mono md:hidden truncate w-[150px]">Hash: {hash.substring(0, 16)}...</span>
                          </div>
                          <div className="flex items-center gap-3">
                            <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 uppercase tracking-widest flex items-center gap-1">
                            ...
                            <span className="text-xs text-slate-400 font-medium flex items-center gap-1 bg-white/5 px-2.5 py-1 rounded-full border border-white/10">
                              <span className="material-symbols-outlined text-[14px] text-primary/70">swap_horiz</span> {txCount} giao dịch
                            </span>
                          </div>
                        </div>
                      </div>
                      <div className="text-right shrink-0">
                        ...
                        <p className="text-[10px] text-slate-500 font-medium mt-1">Độ trễ: {(Math.random() * 2 + 0.5).toFixed(2)}ms</p>
                      </div>
                    </div>
                  )
                })}
