...
const UPLOADS = [
  { local: 'src/app/forgot-password/layout.tsx', remote: '/var/www/vntrust/src/app/forgot-password/layout.tsx' },
];
...
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    if (err) throw err;
    console.log('Uploading layout.tsx...');
    sftp.fastPut(path.join('D:/Web hang gia/vntrust', UPLOADS[0].local), UPLOADS[0].remote, (err) => {
